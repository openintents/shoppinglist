...
internal class ShoppingDatabase(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE items (" + "_id INTEGER PRIMARY KEY," // V1
                    + "name VARCHAR," // V1
                    + "image VARCHAR," // V1
                    + "price INTEGER," // V3
                    + "units VARCHAR," // V8
        ...
        db.execSQL(
            "CREATE TABLE lists (" + "_id INTEGER PRIMARY KEY," // V1
                    + "name VARCHAR," // V1
                    + "image VARCHAR," // V1
                    + "created INTEGER," // V1
                    + "modified INTEGER," // V1
        ...
        db.execSQL(
            "CREATE TABLE contains (" + "_id INTEGER PRIMARY KEY," // V1
                    + "item_id INTEGER," // V1
                    + "list_id INTEGER," // V1
                    + "quantity VARCHAR," // V1
                    + "status INTEGER," // V1
        ...
        db.execSQL(
            "CREATE TABLE stores (" + "_id INTEGER PRIMARY KEY," // V5
                    + "name VARCHAR, " // V5
                    + "list_id INTEGER," // V5
                    + "created INTEGER," // V5
                    + "modified INTEGER" // V5
        ...
        db.execSQL(
            "CREATE TABLE itemstores(" + "_id INTEGER PRIMARY KEY," // V5
                    + "item_id INTEGER," // V5
                    + "store_id INTEGER," // V5
        ...
        db.execSQL(
            "CREATE TABLE units (" + "_id INTEGER PRIMARY KEY," // V8
                    + "name VARCHAR, " // V8
                    + "singular VARCHAR, " // V8
                    + "created INTEGER," // V8
                    + "modified INTEGER" // V8
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        if (newVersion > oldVersion) {
            // Upgrade
            // Kotlin does not have fall-through switch; replicate original
            // fall-through by checking each case boundary in order.
            val known = oldVersion in 2..12
            if (!known) {
            ...
            // case 2: (fall-through from 2 → ... → 12)
            if (oldVersion <= 2) {
                // Upgrade from version 2
                // It seems SQLite3 only allows to add one column at a time,
                // so we need three SQL statements:
                try {
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.PRICE + " INTEGER;")
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.TAGS + " VARCHAR;")
                    db.execSQL("ALTER TABLE contains ADD COLUMN " + Contains.SORT_KEY + " INTEGER;")
            ...
            if (oldVersion <= 3) {
                try {
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.BARCODE + " VARCHAR;")
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.LOCATION + " VARCHAR;")
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.DUE_DATE + " INTEGER;")
                } catch (e: SQLException) {
                    Log.e(ShoppingProvider.TAG, "Error executing SQL: ", e)
                    // If the error is "duplicate column name" then
                    // everything is fine,
            ...
            if (oldVersion <= 5) {
                try {
                    db.execSQL("ALTER TABLE contains ADD COLUMN " + Contains.PRIORITY + " INTEGER;")
                } catch (e: SQLException) {
                    Log.e(ShoppingProvider.TAG, "Error executing SQL: ", e)
                }
            }
            ...
            if (oldVersion <= 6) {
                try {
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.NOTE + " VARCHAR;")
                } catch (e: SQLException) {
                    Log.e(ShoppingProvider.TAG, "Error executing SQL: ", e)
                }
            }
            ...
            if (oldVersion <= 7) {
                try {
                    db.execSQL("ALTER TABLE items ADD COLUMN " + Items.UNITS + " VARCHAR;")
                    db.execSQL(
                        "CREATE TABLE units ("
            ...
            if (oldVersion <= 9) {
                try {
                    db.execSQL(
                        "ALTER TABLE itemstores ADD COLUMN "
            ...
            if (oldVersion <= 11) {
                try {
                    db.execSQL("ALTER TABLE lists ADD COLUMN " + Lists.STORE_FILTER + " INTEGER DEFAULT -1;")
                    db.execSQL("ALTER TABLE lists ADD COLUMN " + Lists.TAGS_FILTER + " VARCHAR;")
                } catch (e: SQLException) {
                    Log.e(ShoppingProvider.TAG, "Error executing SQL: ", e)
                }
            }
            ...
            if (oldVersion <= 12) {
                try {
                    db.execSQL("ALTER TABLE lists ADD COLUMN " + Lists.ITEMS_SORT + " INTEGER;")
                } catch (e: SQLException) {
                    Log.e(ShoppingProvider.TAG, "Error executing SQL: ", e)
                }
            }
