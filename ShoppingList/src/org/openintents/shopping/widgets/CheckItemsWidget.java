...
public class CheckItemsWidget extends AppWidgetProvider {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (extras != null) {
            ...
            if (page != 0) {
                int pagePreference = sharedPreferences.getInt(widgetId + "Page", 0);
                ...
                sharedPreferencesEditor.putInt(widgetId + "Page", pagePreference);
            ...
            if (id != 0) {
                ...
                context.getContentResolver().update(Uri.withAppendedPath(ShoppingContract.Contains.CONTENT_URI, String.valueOf(id)), values,
                        null, null);
            }
        }
    ...
    private void updateWidgets(Context context) {
        ...
        int[] a = appWidgetManager.getAppWidgetIds(new ComponentName(context.getPackageName(), CheckItemsWidget.class.getName()));
    ...
    public void onUpdate(final Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        for (int widgetId : appWidgetIds) {
            ...
            if (listId != -1) {
                RemoteViews updateView = buildUpdate(context, listId, widgetId, page);
                appWidgetManager.updateAppWidget(widgetId, updateView);
            }
        }
    }
    
    public RemoteViews buildUpdate(Context context, long listId, int widgetId, int page) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_check_items);
        ...
        for (int i = 1; i <= LIMIT_ITEMS; i++) {
            int viewId = context.getResources().getIdentifier("item_" + i, "id", context.getPackageName());
            views.setTextViewText(viewId, "");
        }
        
        views.setTextViewText(R.id.item_1, context.getString(R.string.widget_no_items, page + 1));
        ...
        if (cursor.getCount() > 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                int viewId = context.getResources().getIdentifier("item_" + i, "id", context.getPackageName());
                views.setTextViewText(viewId, cursor.getString(cursor.getColumnIndex(ContainsFull.ITEM_NAME)));

                Intent intentCheckService = new Intent(context, CheckItemsWidget.class);
                intentCheckService.putExtra("widgetId", widgetId);
                intentCheckService.putExtra("id", Integer.valueOf(cursor.getString(0)));
                intentCheckService.setAction(ACTION_CHECK);

                PendingIntent pendingIntent = PendingIntent.getBroadcast(context, Integer.valueOf(cursor.getString(0)), intentCheckService, PendingIntent.FLAG_ONE_SHOT);
            ...
            intentGoToApp.setData(Uri.withAppendedPath(ShoppingContract.Lists.CONTENT_URI, "" + listId));
            PendingIntent pendingIntentGoToApp = PendingIntent.getActivity(context, 0, intentGoToApp, PendingIntent.FLAG_UPDATE_CURRENT);
            
            /*
             * List title
             */
            String title = getTitle(context, Uri.withAppendedPath(ShoppingContract.Lists.CONTENT_URI, "" + listId));
             ...
             * Preference button
             */
            Intent intentPreferences = new Intent(context, CheckItemsWidgetConfig.class);
            ...
            PendingIntent pendingIntentPreferences = PendingIntent.getActivity(context, 0, intentPreferences, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            PendingIntent pendingIntentPrevPage = PendingIntent.getBroadcast(context, widgetId, intentPrevPage, PendingIntent.FLAG_ONE_SHOT);
            ...
            PendingIntent pendingIntentNextPage = PendingIntent.getBroadcast(context, widgetId, intentNextPage, PendingIntent.FLAG_ONE_SHOT);
    ...
    private String getTitle(Context context, Uri uri) {
        Cursor c = context.getContentResolver().query(uri, new String[] {ShoppingContract.Lists.NAME}, null, null, null);
    ...
    public static Cursor fillItems(Context context, long listId) {
        String sortOrder = PreferenceActivity.getSortOrderFromPrefs(context, ShoppingActivity.MODE_IN_SHOP);
        String selection = "list_id = ? AND " + ShoppingContract.Contains.STATUS + " == " + ShoppingContract.Status.WANT_TO_BUY;

        Cursor cursor = context.getContentResolver().query(ContainsFull.CONTENT_URI, ShoppingActivity.mStringItems, selection, new String[] { String.valueOf(listId) }, sortOrder);
