...
package org.openintents.shopping.multi;
 ...
 */
public class MultiShoppingActivity extends BaseGameActivity
    ...
    // Debug tag
    final static String TAG = "MultiShoppingActivity";
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.button_got_it:
                // (gameplay) user clicked the "got it" button
                gotShoppingItem();
                break;
        }
    }
    ...
    String mShoppingItem; // next shopping item to collect

    // Reset game variables in preparation for a new game.
    void resetGameVars() {
        mSecondsLeft = GAME_DURATION;
        mShoppingItem = getString(R.string.no_items_available);
    ...
    void startGame() {        
        updateScoreDisplay();
        broadcastShoppingItem(false);
        switchToScreen(R.id.screen_game);

        findViewById(R.id.button_got_it).setVisibility(View.VISIBLE);
    ...
    void gameTick() {
        ...
        if (mSecondsLeft <= 0) {
            // finish game
            findViewById(R.id.button_got_it).setVisibility(View.GONE);
            broadcastEnd(true);
        }
    }

    // indicates the player got the item
    void gotShoppingItem() {
        if (mSecondsLeft <= 0)
            return; // too late!
        mShoppingItem = getString(R.string.no_items_available);
        updateScoreDisplay();
        ...
        // broadcast that I got the item
        broadcastShoppingItem(true);
        broadcastEnd(false);
    }
    ...
    // from the network.
    Map<String, String> mParticipantScore = new HashMap<String, String>();
    ...
    public void onRealTimeMessageReceived(RealTimeMessage rtm) {
        ...
        String ctrChar = msg.substring(0,1);
        String item = msg.substring(1);
        Log.d(TAG, "Message received: " + ctrChar + "/" + mShoppingItem);        

        if ("G".equals(ctrChar) || "N".equals(ctrChar)) {
            // score update.
            String existingItem = mParticipantScore.containsKey(sender) ?
            ...
            if ("G".equals(ctrChar)) {
                ...
                mParticipantScore.put(sender, item);
            }
        ...
        } else if ("S".equals(ctrChar)) {
            // someone else started to play -- so dismiss the waiting room and
            // get right to it!
            Log.d(TAG, "Starting game because we got a start message.");
            dismissWaitingRoom();
            startGame();
        } else if ("E".equals(ctrChar)){
        	findViewById(R.id.button_got_it).setVisibility(View.GONE);
        }
    }

    // Broadcast my score to everybody else.
    void broadcastShoppingItem(boolean gotIt) {        

        // First byte in message indicates whether it's a new shopping item or the user got the item
        mMsgBuf = ((gotIt ? "G" : "N") + mShoppingItem).getBytes();
    ...
    void broadcastStart() {
    	String item = ShoppingUtils.getUncheckedFirstItem(this);
        mMsgBuf = ("S" + item).getBytes();
    ...
    // will react
    // by dismissing their waiting room UIs and starting to play too.
    void broadcastEnd(boolean timeOut) {
        mMsgBuf = "E".getBytes();
    ...
    final static int[] CLICKABLES = {
            ...
            R.id.button_sign_out, R.id.button_got_it
    };
    ...
    void updateScoreDisplay() {
        ((TextView) findViewById(R.id.shopping_item)).setText(mShoppingItem);
    }


    // updates the screen with the scores from our peers
    void updatePeerScoresDisplay() {
        ((TextView) findViewById(R.id.score0)).setText("Me");
        int[] arr = {
        ...
        if (mRoomId != null) {
            for (Participant p : mParticipants) {
                ...
                if (p.getStatus() != Participant.STATUS_JOINED)
                    continue;
                String item = mParticipantScore.containsKey(pid) ? mParticipantScore.get(pid) : "";
                ((TextView) findViewById(arr[i])).setText(p.getDisplayName() + " - " + item);
                ++i;
            }
        }
